...
class SimpleDiagnosticsComponent: public RComponent{
	private:
	//! Name of the topic
	...
	int rosSetup(){
		if(RComponent::rosSetup() == OK){
			...
			freq_diag_ = new diagnostic_updater::FrequencyStatus(diagnostic_updater::FrequencyStatusParam(&min_freq, &desired_freq_, 0.1) );
			
			command_freq_ = new diagnostic_updater::FunctionDiagnosticTask("Topic frequency check", boost::bind(&SimpleDiagnosticsComponent::checkTopicSubscriber, this, _1));
			
			subs_command_freq = new diagnostic_updater::HeaderlessTopicDiagnostic("state", *diagnostic_,
	                    diagnostic_updater::FrequencyStatusParam(&min_topic_freq, &max_topic_freq, 0.1, 10));
