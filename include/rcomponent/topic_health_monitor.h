...
public:
  ...
  //!
  void tick() { last_msg_time_ = ros::Time::now(); }
  //! returns true if the topic is receiving data
  bool isReceiving() { return isReceiving(topic_timeout_); }
  ...
  void setSubscriber(ros::Subscriber *subscriber) { sub_ = subscriber; }

  ros::Subscriber *getSubscriber() { return sub_; }

  void setTimeout(double timeout) { topic_timeout_ = timeout; }

  double getTimeout() { return topic_timeout_; }
}; // class
} // namespace
